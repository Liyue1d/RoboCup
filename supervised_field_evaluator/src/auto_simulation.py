...
# @asyncio.coroutine
# def launch_simulation_async(TEAM_L, TEAM_R, log_directory, N_simulations, N_process, fast_mode=False, idport=0, N_retry=0):
#
#     future = asyncio.get_event_loop().run_in_executor(concurrent.futures.ProcessPoolExecutor(), launch_match, idport,
#                                                          TEAM_L, TEAM_R, log_directory,
#                                                          6000+(idport)*10+N_retry*1000,
#                                                          6001+(idport)*10+N_retry*1000,
#                                                          6002+(idport)*10+N_retry*1000, fast_mode)
...
def launch_match(processID, TEAM_L, TEAM_R, log_directory, port, coach_port, olcoach_port, fast_mode):
    ...
    l = Process(target=launch_team, args=(TEAM_L, port, coach_port, olcoach_port,4,))
    r = Process(target=launch_team, args=(TEAM_R, port, coach_port, olcoach_port,10,))
    l.start()
    r.start()
