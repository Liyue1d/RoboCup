...
Bhv_PenaltyKick::execute( PlayerAgent * agent )
{
    ...
    case GameMode::PenaltyMiss_:
        if ( wm.self().goalie() )
        {
          sRL.send_terminal_signal(agent);
        }
        if ( state->currentTakerSide() == wm.ourSide() )
        {
            if ( wm.self().goalie() )
...
Bhv_PenaltyKick::doKicker( PlayerAgent * agent )
{
    ...
    SingletonRL &sRL = SingletonRL::getInstance();
    Action action = sRL.act(agent);

    std::string action_name (action.name);
    if(action_name.compare(std::string("dash"))==0)
    {
      std::cerr << "TURNNECK ACT" << std::endl;
      agent->doTurnNeck( AngleDeg(action.turn_neck_angle) );
      std::cerr << "DASH ACT" << std::endl;
      agent->doDash(action.dash_power, AngleDeg(action.dash_dir) );
      return true;
    }
    else if(action_name.compare(std::string("kick"))==0)
    {
      std::cerr << "TURNNECK ACT" << std::endl;
      agent->doTurnNeck( AngleDeg(action.turn_neck_angle) );
      std::cerr << "KICK ACT" << std::endl;
      agent->doKick(action.kick_power, AngleDeg(action.kick_direction) );
      return true;
    }
    else if(action_name.compare(std::string("move"))==0)
    {
      std::cerr << "TURNNECK ACT" << std::endl;
      agent->doTurnNeck( AngleDeg(action.turn_neck_angle) );
      std::cerr << "Using move from act!!!" << std::endl;
      agent->doMove(action.move_x, action.move_y);
      return true;
    }
    else if(action_name.compare(std::string("turn"))==0)
    {
      std::cerr << "TURNNECK ACT" << std::endl;
      agent->doTurnNeck( AngleDeg(action.turn_neck_angle) );
      std::cerr << "TURN ACT" << std::endl;
      agent->doTurn( AngleDeg(action.turn_moment) );
      return true;
    }
    else
    {
      std::cerr << "No ACT action!, use C++ IA" << std::endl;
    }
...
Bhv_PenaltyKick::doShoot( PlayerAgent * agent )
{
    ...
    {
        std::cerr << "(doShoot) time limit, Shoot now!!!" << std::endl;
        dlog.addText( Logger::TEAM,
...
Bhv_PenaltyKick::doDribble( PlayerAgent * agent )
{
    ...
    else
    {
        ...
        std::cerr << wm.time() << "Body_Dribble2008" <<  " " << drib_target<<  " " << drib_power<<  " " << drib_dashes<<  " " <<  dodge_mode<<  " "<<std::endl;
    }
